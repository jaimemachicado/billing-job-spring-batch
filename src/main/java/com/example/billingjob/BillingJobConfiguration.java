...
public class BillingJobConfiguration {
    ...
    @Bean
    public Job billingJob(JobRepository jobRepository, Step step1, Step step2, Step step3) {
...
//                .sql(sql)
//                .rowMapper((resultSet, i) -> new BillingData(
//                        resultSet.getInt("data_year"),
//                        resultSet.getInt("data_month"),
//                        resultSet.getInt("account_id"),
//                        resultSet.getString("phone_number"),
//                        resultSet.getFloat("data_usage"),
//                        resultSet.getInt("call_duration"),
//                        resultSet.getInt("sms_count")))
//                .name("billingDataItemReader")
//                .build();
//    }
...
//    }
    ...
    public JdbcCursorItemReader<BillingData> billingDataTableReader(
        ...
        String sql = "select * from BILLING_DATA where DATA_YEAR = %d and DATA_MONTH = %d".formatted(year, month);
        return new JdbcCursorItemReaderBuilder<BillingData>()
    ...
    private JobParametersValidator parameterValidator() {
        return parameters -> {
            if (Objects.isNull(parameters)) {
                throw new IllegalArgumentException("Parameter input.file must be informed.");
            }
            String inputFileName = parameters.getString("input.file");
            if (Objects.isNull(inputFileName) || inputFileName.isBlank()
                    || !inputFileName.endsWith(".csv")) {
                throw new IllegalArgumentException("The input file must be informed and be a CSV file.");
            }
        };
    }
